...
SELECT department_name, round(avg(salary)) as avg_salar
FROM employees
    JOIN departments USING (department_id)
GROUP BY
    department_name
...
SELECT
    EXTRACT(MONTH FROM order_date) AS individual_month,
    EXTRACT(YEAR FROM order_date) AS i_year,
    COUNT(order_id) AS total_orders
FROM orders
WHERE
    EXTRACT(YEAR FROM order_date) = 2022
GROUP BY
    individual_month, i_year;