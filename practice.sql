...
SELECT department_name, round(avg(salary)) as avg_salar FROM employees
JOIN departments USING(department_id)
GROUP BY department_name
ORDER BY avg_salar DESC LIMIT 1;


--Count Employees Hired Each Year
SELECT extract(YEAR FROM hire_date) as Years, count(*) FROM employees
JOIN departments USING(department_id)
GROUP BY Years;
